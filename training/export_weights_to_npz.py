...
if __name__ == "__main__":
    ...
    candidates = [
        os.path.join(_ROOT, "data", "stage1_cnn_centralized.pth"),
        os.path.join(_ROOT, "training", "quickstart-pytorch", "stage1_cnn_centralized.pth"),
        os.path.join(_ROOT, "training", "quickstart-pytorch", "stage1_cnn.pth")
    ]
    pth_found = None
    for c in candidates:
        if os.path.exists(c):
    ...
    if pth_found:
        out_path = os.path.join(_ROOT, "data", "stage1_weights.npz")
        export_model_to_numpy(pth_found, out_path)
    else:
        print("No .pth model found yet. Run training/quickstart-pytorch/train_centralized.py first.")
